...
1. **Apply `0031_invite_code_search_path.sql`** (2026-07-26, PENDING) — `npx supabase db push --linked`, or paste it into the SQL editor. Until this lands, "Create & share code" fails with 42883 and the app falls back to the legacy Family-ID share. Verify with `select proconfig from pg_proc where proname = 'create_invite';` → `{"search_path=public, extensions"}`.
