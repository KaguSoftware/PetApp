...
- **Navigation is real system chrome**: tabs are nested native stacks (`app/(tabs)/<tab>/{_layout,index}.tsx`); pushed screens use the root Stack's native header (back chevron, edge-swipe). Never hand-roll headers/back bars. **ONE header system** (components/Screen.tsx, 2026-07-28 reliability pass): every stack — root, onboarding, all five tab stacks — passes `useNativeHeaderOptions()` unchanged, and every page body is a `TabScreen` (tab top level: collapsing big title + `trailing`→headerRight island) or a `PushedScreen` (title in the bar). No platform forks, no per-screen `setOptions` outside those two, no `headerTitle` string in a stack's `screenOptions` (it silently wins over every screen's own `title`). Both scaffolds set `contentInsetAdjustmentBehavior="never"` and own their padding — the shared bottom allowance (safe area + tab bar via the `InsideTabs` context) is what keeps screens pushed *inside* a tab clear of the tab bar.
